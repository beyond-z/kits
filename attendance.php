...
function get_or_create_event_info_by_name($course_id, $event_name) {
    global $pdo;
    ...
    return get_or_create_event_info_by_name($course_id, $event_name);
}
...
requireLogin();
    ...
    // The event names passed in from the Kit title are HTML encoded. E.g. & and - maybe
    // set to their numeric values. Decode those into safe characters to use in the database
    // for the event_name (which should match the name in the Portal calendar).
    //
    // Unfortunately, we can't just straight decode them b/c the MySQL database uses a 
    // Latin-1 charset, aka 'ISO-8859-1', so for example the character "en dash"
    // code &#8211 isn't a valid Latin-1 character and would just show up as '?' in MySql.
    // So in addition to the regular decoding, imma hack in some special translations for 
    // characters we come across in Kit names. 
    function get_decoded_event_name($event_name){
       // Start by decoding all numerical values that can be converted to Latin-1 charset automatically.
       // E.g. &#038; would be converted to the ampersand b/c it's a Latin-1 char.
       // Note: ENT_COMPAT | ENT_HTML401 is default for how to handle quotes.
       $decoded_event_name = html_entity_decode($event_name, ENT_COMPAT | ENT_HTML401, 'ISO-8859-1'); 

       // Now do the special ones we encounter that are UTF8 and we need to translate to Latin-1

       // Dashes. See: https://en.wikipedia.org/wiki/Dash#Common_dashes_and_Unicode_characters
       $decoded_event_name = str_replace("&#8208;", "-", $decoded_event_name); // unicode hyphen
       $decoded_event_name = str_replace("&#8209;", "-", $decoded_event_name); // non-breaking hyphen
       $decoded_event_name = str_replace("&#8210;", "-", $decoded_event_name); // figure dash
       $decoded_event_name = str_replace("&#8211;", "-", $decoded_event_name); // en dash
       $decoded_event_name = str_replace("&#8212;", "-", $decoded_event_name); // em dash
       $decoded_event_name = str_replace("&#8213;", "-", $decoded_event_name); // horizontal bar
       $decoded_event_name = str_replace("&#8722;", "-", $decoded_event_name); // unicode minus
      
       return $decoded_event_name;
    }
    ...
    if(isset($_GET["event_id"]) && $_GET["event_id"] != "") {
        ...
        $decoded_event_name = get_decoded_event_name($event_name);
        $event_info = get_or_create_event_info_by_name($course_id, $decoded_event_name);
